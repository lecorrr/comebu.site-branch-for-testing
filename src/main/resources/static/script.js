document.addEventListener("DOMContentLoaded", () => {
  ...
  function addToCart(name, price, image, description, id) {
    const existingItem = cartItems.find((i) =>
      typeof id === "number" ? i.id === id : i.name === name
    );
    if (existingItem) {
  ...
  function getOrderItemsPayload() {
    return cartItems
      .filter(
        (item) =>
          typeof item.id === "number" && item.id > 0 && item.quantity > 0
      )
      .map((item) => ({
        productId: item.id,
  ...
  window.getOrderItemsPayload = getOrderItemsPayload;
  ...
  function renderCart() {
    ...
    // корзина не порожня → кнопка активна
    if (checkoutBtn) {
      ...
      .map(
        ...
        <div class="cart-item">
          ...
          <div class="cart-item-info">
            ...
            <button class="cart-description-btn" data-name="${
              item.name
            }">!</button>
          </div>
  ...
  // 👇 ДОДАВ СЮДИ: глобальна функція очищення корзини
    ...
    catalogEl.innerHTML = products
      .map((p) => {
        ...
        <div class="admin-controls admin-only">
          <button class="admin-controls-btn admin-edit-btn"
                  type="button"
                  data-id="${id}"
                  title="Редагувати">
          ...
          <button class="admin-controls-btn admin-delete-btn"
                  type="button"
                  data-id="${id}"
                  title="Видалити">
...
`;
    ...
    catalogEl.addEventListener("click", async (e) => {
      ...
      if (addBtn) {
        ...
        const idAttr = addBtn.dataset.id;
        const id = idAttr ? parseInt(idAttr, 10) : null;
        if (typeof window.addToCart === "function") {
          window.addToCart(name, price, image, description, id);
        }
        return;
      }
      ...
      if (editBtn) {
        ...
        try {
          ...
          const image =
            prompt("Назва зображення (без формату)", product.imageName || "") ||
            "";
          const body = { name, description, weight, price, imageName: image };
...
function getCartTotalFromDom() {
  ...
  const text = (totalNode.textContent || "").replace(/\s/g, "");
...
function getAddressFromDom() {
  ...
  return "Адресу буде уточнено по телефону";
}

(function () {
  const orderModal = document.getElementById("order-modal");
  const checkoutBtn = document.getElementById("checkout-btn");
  if (!orderModal || !checkoutBtn) return;
  ...
  function openOrderModal() {
    const address = getAddressFromDom();
...
(function () {
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", async (e) => {
      ...
      const address = getAddressFromDom().trim();
      const total = getCartTotalFromDom();
      ...
      const getItems = window.getOrderItemsPayload;
      if (typeof getItems !== "function") {
        alert("Не вдалося сформувати замовлення. Оновіть сторінку та спробуйте ще раз.");
        return;
      }
