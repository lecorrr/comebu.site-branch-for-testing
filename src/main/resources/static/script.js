...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form && emailEl && passEl) {
    const isLoginPage =
      document.title.toLowerCase().includes("авторизація") ||
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.error || "Помилка авторизації/реєстрації");
        }
        ...
        alert(err.message || "Сталася помилка. Спробуйте ще раз.");
      }
    });
  }
});
