document.addEventListener("DOMContentLoaded", () => {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener("click", async () => {
      ...
      if (!token || !user) {
        alert("Увійдіть у свій акаунт, щоб оформити замовлення");
        return;
      }
      ...
      if (itemsPayload.length === 0) {
        alert("Не вдалося сформувати замовлення (немає ID товарів)");
        return;
      }
      ...
      try {
        ...
        if (!res.ok) {
          alert(data.error || "Не вдалося створити замовлення");
          return;
        }
        ...
        alert("Помилка мережі при створенні замовлення");
      }
    });
  }
    ...
    catalogEl.addEventListener("click", async (e) => {
      ...
      if (addBtn) {
        ...
        if (typeof window.addToCart === "function") {
          window.addToCart(id, name, price, image, description);
        }
        return;
      }
      ...
      if (editBtn) {
        ...
        try {
          ...
          const image = prompt("Назва зображення (без формату)", product.imageName || "") || "";
